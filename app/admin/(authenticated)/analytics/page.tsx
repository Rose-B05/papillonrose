...
function BarHorizontal({ label, value, max }: { label: string; value: number; max: number }) {
  ...
  return (
    <div className="flex items-center gap-3 text-sm">
      <span className="w-32 md:w-48 truncate text-secondary-text dark:text-white/70 text-xs">{label || "—"}</span>
      <div className="flex-1 h-5 bg-gray-100 dark:bg-neutral-700 rounded-full overflow-hidden">
        <div className="h-full bg-[#c27a72] dark:bg-[#c27a72] rounded-full transition-all" style={{ width: `${pct}%` }} />
      </div>
...
export default function AnalyticsPage() {
  ...
  const exportCSV = () => {
    ...
    a.href = url
    a.download = `analytics-papillon-rose-${period}-${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, "0")}-${String(new Date().getDate()).padStart(2, "0")}.csv`
    a.click()
  ...
  return (
    <div className="min-h-screen bg-[#F8F5F0] dark:bg-neutral-900 p-6 md:p-10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-[#2E2E2E] dark:text-neutral-100 flex items-center gap-2">
              <BarChart3 className="w-6 h-6 text-[#c27a72] dark:text-[#d4968e]" />
              Analytics
            </h1>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={fetchData}
              disabled={loading}
              className="p-2 rounded-xl bg-white dark:bg-neutral-800 border border-black/[0.07] dark:border-white/[0.08] text-secondary-text hover:text-[#c27a72] transition-colors disabled:opacity-50"
              title="Rafraîchir"
            >
            ...
            <a
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium bg-[#c27a72] dark:bg-[#c27a72] text-white hover:bg-[#a86660] dark:hover:bg-[#c27a72] transition-colors"
            >
              Dashboard Vercel
        ...
        {loading && !data && (
          <div className="text-center py-16 text-secondary-text">Chargement des données…</div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-sm border border-black/[0.07] dark:border-white/[0.08]">
                <p className="text-xs text-secondary-text dark:text-white/60 uppercase tracking-wider mb-1">Page vues</p>
                <p className="text-3xl font-bold text-[#2E2E2E] dark:text-neutral-100">{totals.pageviews?.toLocaleString() || "0"}</p>
              </div>
              <div className="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-sm border border-black/[0.07] dark:border-white/[0.08]">
                <p className="text-xs text-secondary-text dark:text-white/60 uppercase tracking-wider mb-1">Visiteurs uniques</p>
                  ...
                  {byDayFiltered.map((d: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1" title={`${d.timestamp?.split("T")[0]}: ${d.pageviews || 0} vues`}>
                        <div className="w-full bg-[#c27a72]/20 dark:bg-[#c27a72]/20 rounded-t relative" style={{ height: `${Math.max(h, 2)}%` }}>
                          <div className="absolute bottom-0 w-full bg-[#c27a72] dark:bg-[#c27a72] rounded-t transition-all" style={{ height: `${h}%` }} />
                        </div>
                      </div>
                    )
                  })}
                </div>
                <div className="flex justify-between mt-2 text-[10px] text-secondary-text dark:text-white/60">
                  ...
                  <div className="space-y-3">
                    {data.byDevice.map((d: any, i: number) => (
                      <div key={i} className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-secondary-text dark:text-white/70 w-20">{d.deviceType || "Inconnu"}</span>
                        <div className="flex-1 h-5 bg-gray-100 dark:bg-neutral-700 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-[#c27a72] dark:bg-[#c27a72] rounded-full transition-all"
