...
export default function BeforeAfterSlider({ before, after, alt }: { before: string; after: string; alt: string }) {
  ...
  return (
    ...
    >
      {/* After image (full) */}
      <img src={img(after)} alt={`${alt} â€” aprÃ¨s`} className="absolute inset-0 w-full h-full object-cover" />

      {/* Before image (clipped) */}
      <div className="absolute inset-0 overflow-hidden" style={{ width: `${pos}%` }}>
        <img src={img(before)} alt={`${alt} â€” avant`} className="absolute top-0 left-0 w-full h-full object-cover" style={{ width: `${100 / (pos / 100)}%` }} />
      </div>
