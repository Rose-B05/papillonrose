...
export async function sendWelcomeEmail(to: string, prenom: string) {
  ...
  await transport.sendMail({
    ...
    html: `<div style="font-family:sans-serif;max-width:600px;margin:auto;padding:20px">
      ...
      <p style="text-align:center;margin:24px 0">
        <a href="${process.env.NEXT_PUBLIC_SITE_URL || "https://www.papillonrose.fr"}/catalogue" style="background:#C9948E;color:#fff;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block">Découvrir le catalogue</a>
      </p>
