  ...
  return (
    <>
      ...
      <button
        ...
        className="md:hidden flex items-center gap-2 w-full bg-white rounded-2xl px-5 py-3.5 border border-black/[0.07] shadow-sm mb-4 text-sm font-medium text-[#2E2E2E]"
      >
        <Filter size={14} className="text-[#C8A97E]" />
        Filtres
        {activeCount > 0 && (
          <span className="ml-auto bg-[#C8A97E] text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center font-bold">
            {activeCount}
          </span>
        )}
            ...
            <div className="px-5 pb-4 space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="flex-1 bg-[#F8F5F0] rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-[#C8A97E]/30"
                />
                <span className="text-[10px] text-gray-400">au</span>
                <input
                  type="date"
...
function SectionHeader({
  ...
  return (
    <button
      onClick={onToggle}
      className="w-full flex items-center justify-between px-5 py-3 text-xs font-semibold text-[#2E2E2E] uppercase tracking-wider hover:bg-[#F8F5F0] transition-colors"
    >
      <span className="flex items-center gap-2">
        {label}
        {count > 0 && (
          <span className="bg-[#C8A97E] text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
            {count}
          </span>
        )}
      </span>
...
function Tag({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span className="inline-flex items-center gap-1.5 bg-[#C8A97E]/10 text-[#C8A97E] text-[11px] font-medium px-2.5 py-1 rounded-full border border-[#C8A97E]/20">
      {label}
      <button
        onClick={onRemove}
        className="hover:bg-[#C8A97E]/20 rounded-full p-0.5 transition-colors"
      >
        <X size={10} />
      </button>
    </span>
