...
const originIcon = makeIcon("#c27a72")
...
function RouteLine({ origin, dest, color = "#c27a72" }: RouteProps) {
  const map = useMap()
...
export default function DeliveryMap({ destination, postalCode, isDark = false }: DeliveryMapProps) {
  ...
  return (
    <div className="relative w-full rounded-xl overflow-hidden border border-black/[0.05] dark:border-white/[0.08]">
      {loading && (
        <div className="h-[200px] bg-gradient-to-br from-[#F0EBE3] to-white dark:from-neutral-800 dark:to-neutral-900 flex items-center justify-center">
          <div className="flex flex-col items-center gap-2">
            <div className="w-5 h-5 border-2 border-[#c27a72] border-t-transparent rounded-full animate-spin" />
            <p className="text-xs text-secondary-text dark:text-white/60">Chargement de la carte…</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="h-[200px] bg-gradient-to-br from-[#F0EBE3] to-white dark:from-neutral-800 dark:to-neutral-900 flex items-center justify-center">
          <p className="text-xs text-secondary-text dark:text-white/60">Adresse non localisable</p>
        </div>
      )}
