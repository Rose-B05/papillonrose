  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 left-6 z-50 w-12 h-12 bg-white border border-[#C8A97E]/40 text-[#C8A97E] rounded-full shadow-lg flex items-center justify-center hover:bg-[#C8A97E]/10 hover:border-[#C8A97E] transition-all"
      >
      ...
      {open && (
        <div
          ...
          className="fixed bottom-20 left-6 z-50 w-72 bg-white rounded-2xl shadow-2xl border border-black/[0.07] p-5 animate-[fade-in-up_0.2s_ease-out]"
        >
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm font-semibold text-[#2E2E2E]">Accessibilité</p>
            <button
              ref={firstFocusRef}
              onClick={() => { setOpen(false); buttonRef.current?.focus() }}
              aria-label="Fermer le panneau d'accessibilité"
              className="w-7 h-7 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-400 hover:text-[#2E2E2E] transition-colors"
            >
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-9 h-9 rounded-lg border border-black/[0.08] flex items-center justify-center text-[#2E2E2E] hover:bg-[#F0EBE3] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="5" y1="12" x2="19" y2="12" /></svg>
                </button>
                <span className="text-sm font-medium text-[#2E2E2E] w-12 text-center">{settings.fontSize}%</span>
                <button
            ...
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm text-[#2E2E2E]">Contraste élevé</span>
              <button
                role="switch"
                aria-checked={settings.highContrast}
                onClick={() => update({ highContrast: !settings.highContrast })}
                className={`relative w-10 h-5 rounded-full transition-colors ${settings.highContrast ? "bg-[#C8A97E]" : "bg-gray-200"}`}
              >
            ...
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm text-[#2E2E2E]">Réduire les animations</span>
              <button
                role="switch"
                aria-checked={settings.reduceAnimations}
                onClick={() => update({ reduceAnimations: !settings.reduceAnimations })}
                className={`relative w-10 h-5 rounded-full transition-colors ${settings.reduceAnimations ? "bg-[#C8A97E]" : "bg-gray-200"}`}
              >
            ...
            {/* Police dyslexie */}
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm text-[#2E2E2E]">Police adaptée dyslexie</span>
              <button
                role="switch"
                aria-checked={settings.dyslexiaFont}
                onClick={() => update({ dyslexiaFont: !settings.dyslexiaFont })}
                className={`relative w-10 h-5 rounded-full transition-colors ${settings.dyslexiaFont ? "bg-[#C8A97E]" : "bg-gray-200"}`}
              >
